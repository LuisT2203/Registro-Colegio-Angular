...
descargarPdf(id_personaE: number) {
  console.log('ID Personal recibido:', id_personaE);
  if (!id_personaE) {
    console.error('ID Personal no definido');
    return;
  }
  this.service.exportPdfById(id_personaE).subscribe((data: Blob) => {
    const blob = new Blob([data], { type: 'application/pdf' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `ReportePersonal_${id_personaE}.pdf`;
    a.click();
